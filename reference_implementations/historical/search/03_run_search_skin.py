...
#
# Private archive source: PDARTS/Github/master/03_run_search_skin.py
# Original SHA-256: ba0298a3bd487cbcbb69ee2601d742df070b395ccfd217e6bbd80ced2a897a54
#
...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 18 15:16:49 2025
...
# -*- coding: utf-8 -*-
"""
032_run_search_skin_BATCH.py

Batch automation wrapper for your existing P-DARTS search script.

What it adds:
- loops over search_depth_n in a list (e.g., 6,8,10)
- loops over multiple seeds (e.g., 3 seeds => 3 genotypes per n)
- per-run outputs remain identical to your current script
- writes a master batch ledger: _BATCH_LEDGER.csv
- supports resume/skip existing, continue on error, dry-run
"""
...
DEFAULT_SAVE_ROOT = "PROJECT_ROOT/output/search_runs/256"
DEFAULT_RUN_NAME  = "ISIC_search"
# ---------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("P-DARTS search on skin lesion datasets (batch enabled)")
    ...
    # ---------- Single-run args ----------
    p.add_argument("--search_depth_n", type=int, choices=[6, 8, 10], default=6)

    # Manuscript uses 25 per stage (you can keep 2 for smoke test)
    ...
    p.add_argument("--batch_name", type=str, default="batch_search",
                   help="Name used in _BATCH_LEDGER.csv")
    p.add_argument("--search_depth_list", type=str, default="6,8,10",
                   help="Comma-separated list of n values to run in batch mode, e.g. 6,8,10")
    p.add_argument("--seeds", type=str, default="0,1,2",
                   help="Comma-separated list of seeds (3 seeds => 3 genotypes per n).")
...
def train_one_epoch(train_queue, valid_queue, model, network_params,
    ...
    # ---- FIX: support both DataParallel and plain model ----
    ...
    for step, (x, y) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            logits_a = model(xs)              # works for both DP and non-DP
...
def _make_run_dir(args: argparse.Namespace) -> Path:
    ...
    ts = _now_stamp()
    run_dir = Path(args.save_root) / f"{slug}__{ts}"
    return run_dir


def _find_existing_run_dir(save_root: Path, signature_prefix: str) -> Optional[Path]:
    """
    If skip_existing is enabled, we match based on signature_prefix which contains
    run_name/dataset/n/seed/img/bs/lr/wd/archlr/eps... and ignore timestamp suffix.
    Returns the most recently modified matching run dir, if any.
    """
    if not save_root.exists():
        return None
    matches = []
    for p in save_root.glob(f"{signature_prefix}__*"):
        if p.is_dir():
            matches.append(p)
    if not matches:
        return None
...
def run_search_once(args: argparse.Namespace) -> Tuple[Path, str]:
    """
    Runs ONE search and returns (run_dir, genotype_str).
    """
    ...
    model = None  # keep last stage model accessible for final
    for sp in range(3):
        ...
        for ep in range(args.epochs_per_stage):
            ...
            if (args.epochs_per_stage - (ep + 1)) < 5:
                val_acc, val_loss = validate(valid_q, model if use_cuda else model, criterion, args.report_freq, device)
    ...
    logging.info("FINAL GENOTYPE (manuscript aligned): %s", genotype_str)
    ...
    row = {
        ...
        "stage_layers": ",".join(map(str, [args.search_depth_n - 4, args.search_depth_n - 2, args.search_depth_n])),
...
def run_batch(args: argparse.Namespace) -> None:
    ...
    for i, (n, sd) in enumerate(planned, start=1):
        ...
        # Create a per-run args clone
        run_args = copy.deepcopy(args)
        run_args.search_depth_n = int(n)
        run_args.seed = int(sd)

        # optional: skip existing by signature prefix
        signature_prefix = (
        ...
        print(f"[{i}/{len(planned)}] RUN: n={n} seed={sd}")
        try:
        ...
        # GPU cleanup between runs (important for large batches)
        try:
...
def main():
    ...
    # AUTO-BATCH: if user provided multiple n or multiple seeds, run batch automatically
    ns = _parse_int_list_csv(args.search_depth_list)
    seeds = _parse_int_list_csv(args.seeds)
    ...
    if (len(ns) > 1) or (len(seeds) > 1):
        ...
        # fallback single-run behavior
        args.search_depth_n = ns[0] if ns else args.search_depth_n
